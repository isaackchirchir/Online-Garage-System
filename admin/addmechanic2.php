...
<?php include('header.php'); ?>
	...
	<div class="recipient_reg" style="background-color:#272327;">
		<h3 class="text-center" style="background-color:#272327;color: #fff;">Add Mechanic</h3>
		...
		<form enctype="multipart/form-data" action=""  method="post" class="text-center" style="margin-left: 110px">
			 <div class="col-md-12">
					...
					<button name="submit" type="submit" style="margin-left:148px;margin-top: 4px;width:95px;border-radius: 3px;height: 30px">Add Mechanic</button> <br>
					...
					<?php  
						 if(isset($_POST['submit'])){
							...
							if(move_uploaded_file($_FILES["pic"]["tmp_name"], $target_file)) {
									include('../config.php');
									$sql1 = "SELECT * FROM mechanic WHERE userid='".$_POST["userid"]."' OR email= '" . $_POST["email"] . "' ";
	              					$result = $conn->query($sql1);
	              					if($result->num_rows > 0){
	              						 echo "<script>alert('Sorry, Userid or E-mail already exist!');</script>";
	              					}
	              					else{
									$sql = "INSERT INTO mechanic (name,address,contact,email,expertise,userid,password,pic)
										...
										if ($conn->query($sql) === TRUE) {
										    echo "<script>alert('New Mechanic Has been Added Successfully!');</script>";
										} else {
